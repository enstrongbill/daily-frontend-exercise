# daily-frontend-exercise
## This project is used to record some front-end effects and interview questions which encounter in learning
## 这个项目是用于记录一些学习时候遇到的有趣的前端特效和面试题
## 前端特效大多是重构了codepen中的代码
## 每个前端特效有多种实现方式，这里并的代码并不是最优的，并没有兼容各种设备，考虑最优性能等，这里只是提供一种解决问题的思路
## 面试题是不分初中高级面试题的[个人水平有限，无法分辨出]，只要有趣就记录

主要来源(感谢以下书籍或是网站给予个人的帮助)
1. [css secrets](https://book.douban.com/subject/26295140/)
2. [css 世界](https://book.douban.com/subject/27615777/)
3. [svg动画](https://book.douban.com/subject/30239781/)
4. [coden](https://codepen.io/)
5. [前端每日实战](https://segmentfault.com/blog/comehope)
6. [css3patterns](http://lea.verou.me/css3patterns/)
7. [animate.css](https://github.com/daneden/animate.css)
8. [ihover.css](http://gudh.github.io/ihover/dist/index.html)
9. [dynamicsjs](http://dynamicsjs.com/)
10. [mo.js](http://mojs.io/)
11. [dribble](https://dribbble.com/)
12. [vivus](https://maxwellito.github.io/vivus/)
13. [css-gradient](http://bennettfeely.com/gradients/)

